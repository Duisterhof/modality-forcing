...
def read_depth_map(path: str) -> np.ndarray:
    ...
    if path.endswith(".npy"):
        ...
        if depth.dtype == np.uint8:
            print(
                "[depth] WARNING: 8-bit input — depth is heavily quantized. "
                "Prefer .npy or a 16-bit PNG/TIFF.",
                file=sys.stderr,
            )
    depth = np.asarray(depth, dtype=np.float32)
    if depth.ndim == 3:
        print(
            "[depth] WARNING: multi-channel input — using channel 0. If this "
            "is a colormapped depth visualization the result will be wrong; "
            "pass the raw depth (.npy or 16-bit PNG/TIFF).",
            file=sys.stderr,
        )
        depth = depth[..., 0]
    return depth
